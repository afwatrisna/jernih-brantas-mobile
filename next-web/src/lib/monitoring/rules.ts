...
export function getStationInsight(station: StationState, readings: Reading[]): StationInsight {
  ...
  const persistent = recent.length >= 3 && recent.slice(-3).every((r) => r.ntu > Math.max(25, station.baseline * 1.4));
  ...
  const hadAlert = readings.some((r) => ["high", "critical"].includes(getSeverity(r.ntu, station.baseline)));
